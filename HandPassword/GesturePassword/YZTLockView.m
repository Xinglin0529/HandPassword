...
- (void)drawRect:(CGRect)rect
{
    ...
    //添加圆形路径
    //遍历所有的itemView
    [_itemViewsM enumerateObjectsUsingBlock:^(YZTItemView *itemView, NSUInteger idx, BOOL *stop) {
...
- (void)lockViewPrepare
{
    for (NSUInteger i=0; i<9; i++) {
        YZTItemView *itemView = [[YZTItemView alloc] init];
...
- (void)layoutSubviews
{
    ...
    CGFloat itemViewWH = kScreenWidth / 6;
    CGFloat marginValue = itemViewWH / 2;
    ...
    [self.subviews enumerateObjectsUsingBlock:^(UIView *subview, NSUInteger idx, BOOL *stop) {
        ...
        CGFloat x = itemViewWH + (itemViewWH + marginValue) * row;
        CGFloat y = itemViewWH + (itemViewWH + marginValue) * col;
        frame = CGRectMake(x, y, itemViewWH, itemViewWH);
        //设置tag
        subview.tag = idx;
        subview.frame = frame;
    }];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CoreLockTypeSetPwd == _coreLockType) { // 设置密码
        //开始
    ...
    } else if (CoreLockTypeVeryfiPwd == _coreLockType){//验证密码
        //开始
        if (_verifyPWBeginBlock != nil) _verifyPWBeginBlock();
        
    } else if (CoreLockTypeModifyPwd == _coreLockType){
        //开始
...
- (void)gestureEnd
{
    ...
    for (YZTItemView *itemView in self.itemViewsM) {
...
- (void)setpwdCheck
{
    ...
    } else if (CoreLockTypeModifyPwd == _coreLockType) {//修改密码
...
- (void)lockHandle:(NSSet *)touches
{
    ...
    YZTItemView *itemView = [self itemViewWithTouchLocation:loc];
...
- (void)calDirect
{
    ...
    //取出最后一个对象
    YZTItemView *last_1_ItemView = _itemViewsM.lastObject;
    
    //倒数第二个
    YZTItemView *last_2_ItemView =_itemViewsM[count - 2];
 ...
 *  item处理
 */
- (void)itemHandel:(YZTItemView *)itemView
{
    //选中
...
- (YZTItemView *)itemViewWithTouchLocation:(CGPoint)loc
{
    YZTItemView *itemView = nil;
    for (YZTItemView *itemViewSub in self.subviews) {
